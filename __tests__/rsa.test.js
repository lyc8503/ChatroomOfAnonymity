import {
  ...
  rsaBlindMask,
  rsaBlindUnmask,
} from "../pages/api/cryptography/rsa";
...
describe("RSA", () => {
  ...
  it("Miller Rabin primality test", () => {
    ...
    expect(
      isProbablyPrime(393050634124102232869567034555427371542904833n, 20),
    ).toBe(true);
    expect(isProbablyPrime(263130836933693530167218012159999999n, 20)).toBe(
      true,
    );
    expect(
      isProbablyPrime(
        393050634124102232869567034555427371542904833n *
          263130836933693530167218012159999999n,
        20,
      ),
    ).toBe(false);
    ...
    expect(
      isProbablyPrime(
        85242211202858786730267154491553963655327912877020919193394117550602885820056229703249885100818088842279128529357190087962009781n,
        20,
      ),
    ).toBe(true);
    expect(
      isProbablyPrime(
        71975181411965726322510095541947543238255795997210144129532569827028116934092531868433403311550768820891943390065129490369889451n,
        20,
      ),
    ).toBe(true);
    expect(
    ...
    expect(
      isProbablyPrime(
        ,
        20,
      ),
    ).toBe(true);
    expect(
  ...
  it("should encrypt and decrypt", () => {
    ...
    for (let i = 0; i < 1000; i++) {
      ...
      const m = BigInt(
        "0x" + buf.reduce((s, b) => s + b.toString(16).padStart(2, "0"), ""),
      );

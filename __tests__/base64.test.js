import {
  ...
  base64UrlEncode,
  base64UrlDecode,
} from "../pages/api/cryptography/base64";
...
describe("Base64 Decode", () => {
  ...
  it("should throw error decoding wrong padding", () => {
    expect(() => base64Decode("SGVsbG8gV29ybGQh=")).toThrowError(
      "Invalid base64 string padding",
    );
  });

  it("should throw error decoding invalid string", () => {
    expect(() => base64Decode("Hello World!")).toThrowError(
  ...
  it("random decode test", () => {
    for (let cnt = 0; cnt < 500; cnt++) {
      ...
      const decoded = base64UrlDecode(
        Buffer.from(randomTest).toString("base64url"),
      );
